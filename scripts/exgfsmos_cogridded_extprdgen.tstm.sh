#!/bin/sh
##########################################################################
#  Job Name: exgfsmos_cogridded_extprdgen.tstm.sh.ecf 
...
##########################################################################
#
PS4='cogridded_extprdgen.tstm $SECONDS +'
#PS4='${PMI_FORK_RANK} $SECONDS ${0/\/gpfs\/hps\/nco\/ops\/nw.*\/gfs_mos.*\/scripts/} +'
set -x

echo MDLLOG: `date` - Begin job exgfsmos_cogridded_extprdgen.tstm

cd $DATA/cogridded/tstm
cpreq $DATA/ncepdate .
...
############################################################################
#
if [ ! -f $COMIN/mdl_gfsgmosco.tstm.$cycle ]
     then echo 'need successful run of gfsmos_cogridded_prdgen.tstm. to run properly' >> $pgmout
             export err=1;err_chk
fi

cpreq $COMIN/mdl_gfsgmosco.tstm.$cycle .
...
export FORT31="$FIXgfs_mos/mdl_gfsxvect2grid_tsvr40km.in.$cycle"
export FORT32="$FIXgfs_mos/mdl_gfsxvect2grid_tsvr40km.out.$cycle"
export FORT29="$FIXcode/mdl_mos2000id.tbl"
export FORT48="mdl_gfstsvr40.$cycle"
export FORT60="mdl_gfsxtsvr40grd.sq.$cycle"
$EXECcode/mdl_vect2grid < $PARMgfs_mos/mdl_vect2grid_tsvr40km.cn >> $pgmout 2>errfile
export err=$?; err_chk
...
startmsg
export FORT10="ncepdate"
export FORT25="mdl_gfsxtsvr40grd.sq.$cycle"
export FORT28="$FIXcode/mdl_mos2000id.tbl"
export FORT27="$FIXgfs_mos/mdl_gfsxgrd2grd_tsvr40.ids.$cycle"
export FORT42="mdl_gfsgmosco.tstm.$cycle"
...
export FORT28="$FIXgfs_mos/mdl_gfsxgrpost_co.tstm.$cycle"
export FORT29="$FIXcode/mdl_mos2000id.tbl"
export FORT42="mdl_gfsgmosco.tstm.$cycle"
...
#######################################################################
echo MDLLOG: `date` - Job gfsmos_cogridded_extprdgen.tstm has ended.
#######################################################################
