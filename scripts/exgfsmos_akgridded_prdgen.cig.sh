#!/bin/sh
#######################################################################
#  Job Name: exgfsmos_akgridded_prdgen.cig.sh.ecf 
...
#
set -x
PS4='akgridded_prdgen.cig $SECONDS +'
#PS4='${PMI_FORK_RANK} $SECONDS ${0/\/gpfs\/hps\/nco\/ops\/nw.*\/gfs_mos.*\/scripts/} +'
echo MDLLOG: `date` - Begin job exgfsmos_akgridded_prdgen.cig
...
export FORT28="$FIXgfs_mos/mdl_gfsramerge_ak.cig.$cycle"                           #INPUT VARIABLE LIST  ?
...
export FORT60="mdl_gfsmergesta_ak.cig.$cycle"
...
#######################################################################
cpreq $FIXcode/mdl_rafile_template mdl_gfsgmosak.cig.$cycle
...
export FORT38="$FIXgfs_mos/mdl_gfsgranlids_ak.cig.$cycle"                          #INPUT ID LIST
export FORT44="$FIXgfs_mos/mdl_analysisgrconst_ak"                                 #INPUT GRIDDED CONSTANT FILE
export FORT63="$FIXgfs_mos/mdl_u405acigakcn"                                       #INPUT CIG CONTROL FILE
...
export FORT26="$FIXgfs_mos/mdl_granlsta_ak.lst"                                    #INPUT STATION LIST 
export FORT27="$FIXgfs_mos/mdl_granlsta_ak.tbl"                                    #INPUT STATION TABLE
export FORT28="$FIXgfs_mos/mdl_gfsgrpost_ak.cig.$cycle"                            #INPUT ID LIST
export FORT29="$FIXcode/mdl_mos2000id.tbl"                                     #INPUT MOS2000 ID TABLE
export FORT42="mdl_gfsgmosak.cig.$cycle"                                       #
export FORT60="mdl_gfsgmosaksq_pp.cig.$cycle"                                  #  
startmsg
...
if test $SENDCOM = 'YES'
then
  cpfs mdl_gfsmergesta_ak.cig.$cycle $COMOUT
  cpfs mdl_gfsgmosak.cig.$cycle $COMOUT
  cpfs mdl_gfsgmosaksq_pp.cig.$cycle $COMOUT
fi
#
#######################################################################
