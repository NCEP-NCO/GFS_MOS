#!/bin/sh
#######################################################################
#  Job Name: exgfsmos_cogridded_prdgen.vis.sh.ecf 
...
#######################################################################
#
PS4='cogridded_prdgen.vis $SECONDS +'
#PS4='${PMI_FORK_RANK} $SECONDS ${0/\/gpfs\/hps\/nco\/ops\/nw.*\/gfs_mos.*\/scripts/} +'
set -x
echo MDLLOG: `date` - Begin job exgfsmos_cogridded_prdgen.vis

cd $DATA/cogridded/vis
cpreq $DATA/ncepdate .
...
cpreq $FIXcode/mdl_rafile_template mdl_grd2grd_co.vis.$cycle
...
export FORT28="$FIXgfs_mos/mdl_gfsramerge_co.vis.$cycle"                           #INPUT VARIABLE LIST
...
export FORT60="mdl_gfsmergesta_co.vis.$cycle"
...
export FORT27="$FIXgfs_mos/mdl_gfsgrd2grd_vis.ids.$cycle"                          #INPUT ID LIST
export FORT42="mdl_grd2grd_co.vis.$cycle"
$EXECcode/mdl_grd2grd < $PARMgfs_mos/mdl_grd2grd_co.cn >> $pgmout 2>errfile
export err=$?; err_chk
echo MDLLOG: `date` - GRD2GRD ended
$EXECcode/itdlp mdl_grd2grd_co.vis.$cycle -tdlp  mdl_grd2grd_co_sq.vis.$cycle
export err=$?; err_chk
#$EXECcode/itdlp mdl_grd2grd_co_ra.vis.$cycle -append -tdlpra mdl_gfsgmosco.vis.$cycle 
#######################################################################
#
...
#######################################################################
cpreq $FIXcode/mdl_rafile_template_large mdl_gfsgmosco.vis.$cycle
...
export FORT17="$FIXgfs_mos/mdl_gmoscobogusfile.vis"                                #INPUT BOGUS LIST
export FORT18="$FIXgfs_mos/mdl_stationradii_co.vis"                                #INPUT RADII FILE
export FORT26="$FIXgfs_mos/mdl_granlsta_co_vis.lst"                                #INPUT STATION LIST
export FORT27="$FIXgfs_mos/mdl_granlsta_co.tbl"                                    #INPUT STATION TABLE
export FORT29="$FIXcode/mdl_mos2000id.tbl"                                     #INPUT MOS2000 ID TABLE
export FORT30="mdl_grd2grd_co_sq.vis.$cycle"                                   #OUTPUT SEQUENTIAL FILE
export FORT32="mdl_gfspkd.$cycle"
export FORT36="$FIXgfs_mos/mdl_gmoscobogusfile.vis"                                #INPUT BOGUS LIST
export FORT37="$FIXgfs_mos/mdl_granlstation_copairs"                               #INPUT STATION PAIRS
export FORT38="$FIXgfs_mos/mdl_gfsgranlids_co.vis.$cycle"                          #INPUT ID LIST
export FORT44="$FIXgfs_mos/mdl_analysisgrconst_co"                                 #INPUT GRIDDED CONSTANT FILE
export FORT63="$FIXgfs_mos/mdl_u405aviscocn"                                       #INPUT VIS CONTROL FILE
export FORT80="mdl_gfsmergesta_co.vis.$cycle"
export FORT81="mdl_gfsxmergesta_co.$cycle"
export FORT42="mdl_gfsgmosco.vis.$cycle"
...
export FORT26="$FIXgfs_mos/mdl_granlsta_co.lst"                                    #INPUT STATION LIST
export FORT27="$FIXgfs_mos/mdl_granlsta_co.tbl"                                    #INPUT STATION TABLE
export FORT28="$FIXgfs_mos/mdl_gfsgrpost_co.vis.$cycle"                            #INPUT ID LIST
export FORT29="$FIXcode/mdl_mos2000id.tbl"                                     #INPUT MOS2000 ID TABLE
export FORT42="mdl_gfsgmosco.vis.$cycle"
...
then
  ## TDLPACK FILES
  cpfs mdl_gfsmergesta_co.vis.$cycle $COMOUT
  cpfs mdl_gfsgmosco.vis.$cycle $COMOUT
fi
#
#######################################################################
echo MDLLOG: `date` - Job exgfsmos_cogridded_prdgen.vis has ended.
#######################################################################
